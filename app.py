def process_drm(buffer, positions):
    ...
    for fopn_pos in positions:
        try:
            # Extraire un contexte large autour de FOPN
            start_pos = max(0, fopn_pos - 100)
            end_pos = min(len(buffer), fopn_pos + 1000)
            context = buffer[start_pos:end_pos].decode('latin-1', errors='ignore')
            
            # 1. Position de FOPN relative au contexte
            rel_fopn = fopn_pos - start_pos
            
            # 2. Remplacer FOPN par FlateDecode (position absolue)
            st.write(f"Remplacement FOPN à {fopn_pos}")
            processed[fopn_pos:fopn_pos+18] = b'/Filter/FlateDecode'
            
            # 3. Trouver et modifier V=1 (position relative puis absolue)
            v_pos = context.find('/V 1', rel_fopn)
            if v_pos != -1:
                abs_v_pos = start_pos + v_pos + 3  # +3 pour pointer sur le '1'
                st.write(f"Modification V=1 à {abs_v_pos}")
                processed[abs_v_pos] = ord('0')
            else:
                st.warning("V=1 non trouvé")
                
            # 4. Trouver et effacer le stream chiffré
            info_pos = context.find('/INFO(', rel_fopn)
            if info_pos != -1:
                endstream_pos = context.find('endstream', info_pos)
                if endstream_pos != -1:
                    # Convertir en positions absolues
                    abs_info_start = start_pos + info_pos
                    abs_stream_end = start_pos + endstream_pos
                    
                    st.write(f"Effacement stream {abs_info_start}-{abs_stream_end}")
                    processed[abs_info_start:abs_stream_end] = b'\x00' * (abs_stream_end - abs_info_start)
                else:
                    raise ValueError("endstream non trouvé")
            else:
                raise ValueError("INFO non trouvé")
